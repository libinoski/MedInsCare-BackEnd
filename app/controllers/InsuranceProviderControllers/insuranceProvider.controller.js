...
exports.login = async (req, res) => {
  ...
  try {
    ...
    if (error.message === "Inurance provider not found" || error.message === "Wrong password" || error.message === "Please wait for approval") {
      return res.status(422).json({
        status: "failed",
  ...
  jwt.verify(
    ...
    async (err, decoded) => {
      ...
      try {
        ...
        const validationResults = validateInsuranceProviderChangePassword();
        if (!validationResults.isValid) {
          return res.status(400).json({
            status: "failed",
            message: "Validation failed",
